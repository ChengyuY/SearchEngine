...
export class KmpComponent implements OnInit {
  ...
  recherche(){
    console.log("Search classement kmp:",this.text)
    this.load = true
    if (this.text != null) {
      console.log("Book list now:", this.bookList)
      // @ts-ignore
      this.reService.recherche_kmp(this.text,this.curPage,this.pageSize).subscribe((res: HttpResponse<Book[]>) => {
        localStorage.setItem("text-c-token",this.text)
        // @ts-ignore
  ...
  ngOnInit(): void {
    // @ts-ignore
    this.text = localStorage.getItem('text-c-token')
    //this.recherche()
  }
}
